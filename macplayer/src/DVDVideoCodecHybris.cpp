...
CDVDVideoCodecHybris::CDVDVideoCodecHybris() : CDVDVideoCodec() {
  ...
  memset(&m_hints, 0, sizeof(CDVDStreamInfo));
  ///m_bitstream = NULL;
  m_render_sw = false;
  m_surface = NULL;
  m_textureId = 0;
...
CDVDVideoCodecHybris::~CDVDVideoCodecHybris() {
  INFO( "%s::%s::%d - LOG", CLASSNAME, __func__, __LINE__);
  Dispose();
  INFO( "%s::%s::%d - LOG", CLASSNAME, __func__, __LINE__);
}

///bool CDVDVideoCodecHybris::Open(CDVDStreamInfo &hints/*, CDVDCodecOptions &options*/) {
bool CDVDVideoCodecHybris::Open(CDVDStreamInfo &hints, uint8_t* _data, int _size) {
  int ret = 0;
  ...
  DBG( "%s::%s - trying to open, codec(%d), profile(%d), level(%d)",
       CLASSNAME, __func__, hints.codec, hints.profile, hints.level);

  m_drop = false;
  ///m_hints = hints;
  memcpy(&m_hints,&hints,sizeof(CDVDStreamInfo));
  if(hints.extrasize > 0) {
    m_hints.extradata = malloc(FF_INPUT_BUFFER_PADDING_SIZE+hints.extrasize);
    memset(m_hints.extradata, 0, FF_INPUT_BUFFER_PADDING_SIZE+hints.extrasize);
  ...
  if(_size > 0) {
    m_hints.extradata = malloc(FF_INPUT_BUFFER_PADDING_SIZE+_size);
    memset(m_hints.extradata, 0, FF_INPUT_BUFFER_PADDING_SIZE+_size);
    memcpy(m_hints.extradata, _data, _size);
    m_hints.extrasize = _size;
  } else {
    m_hints.extradata = NULL;
    m_hints.extrasize = 0;
  }
...
//    OMX_ON2_VIDEO_CodingVP9,        /**< VP9 */^M
    default:
      DBG( "%s::%s - Unsupported hints.codec(%d)", 
  ...
  if (m_format == NULL) {
    ERROR( "%s::%s - Failed to create format object for %s", 
  ...
  if (m_codec == NULL) {
    ERROR( "%s::%s - Failed to create codec for %s", 
              CLASSNAME, __func__, m_mimeType.c_str());
  ...
  if( m_render_sw ) {
    if (media_codec_configure(m_codec, m_format, NULL, 0) != OK) {
      INFO( "%s::%s - Failed to configure codec for %s", 
                CLASSNAME, __func__, m_mimeType.c_str());
      return false;
    }
  } else {
    if (media_codec_configure(m_codec, m_format, m_surface, 0) != OK) {
      INFO( "%s::%s - Failed to configure codec w surface for %s",
                CLASSNAME, __func__, m_mimeType.c_str());
      return false;
    }
  }
...
void CDVDVideoCodecHybris::Dispose() {
  DBG( "%s::%s - Freeing memory allocated for buffers", CLASSNAME, __func__);
...
int CDVDVideoCodecHybris::Decode(BYTE* pData, int iSize, double dts, double pts) {
  ...
  if (pData)
  {
    ...
    if (ret == OK)
    {
      ...
      {
        ERROR( "CDVDVideoCodecHybris::Decode, iSize(%d) > size(%d)", iSize, size);
...
*/
      ...
      bufferInfo.presentation_time_us = presentationTimeUs;
      bufferInfo.flags = 0;

      if(media_codec_queue_input_buffer(m_codec, &bufferInfo) != OK) {
        ERROR( "CDVDVideoCodecHybris::Decode, failed to queue input buffer!");
      }
    }
  }
...
void CDVDVideoCodecHybris::Reset() {
  ...
  if (m_codec)
  {
    ...
    if(media_codec_flush(m_codec) != OK) {
        INFO( "CDVDVideoCodecHybris::Reset(), media_codec_flush failed!");
    }
...
int CDVDVideoCodecHybris::GetOutputPicture(void) {
  ...
  if (ret == OK)
  {
    ...
    if (flags & CJNIMediaCodec::BUFFER_FLAG_SYNC_FRAME)
      DBG( "CDVDVideoCodecHybris:: BUFFER_FLAG_SYNC_FRAME");

    if (flags & CJNIMediaCodec::BUFFER_FLAG_CODEC_CONFIG)
      DBG( "CDVDVideoCodecHybris:: BUFFER_FLAG_CODEC_CONFIG");
#endif
    if (flags & 4 /*CJNIMediaCodec::BUFFER_FLAG_END_OF_STREAM*/)
    {
      DBG( "CDVDVideoCodecHybris:: BUFFER_FLAG_END_OF_STREAM");
      media_codec_release_output_buffer(m_codec, index, 0);
      return 0;
    }
    ...
    //DBG( "CDVDVideoCodecHybris::GetOutputPicture "
...
//    m_output = m_codec->getOutputBuffers();
  ...
  else
  {
    ...
    ERROR( "CDVDVideoCodecHybris::GetOutputPicture unknown index(%d)", index);
  }
...
void CDVDVideoCodecHybris::OutputFormatChanged(void) {
  ...
  if (!m_render_sw) {
    INFO( "CDVDVideoCodecHybris:: Direct Surface Rendering");
    ...
    if (color_format == HAL_PIXEL_FORMAT_YCrCb_NV12 /* RK dedicated: 32 */) {
      INFO( "CDVDVideoCodecHybris::HAL_PIXEL_FORMAT_YCrCb_NV12 = 0x20");
      ...
      ERROR( "CDVDVideoCodecHybris:: Fixme unknown color_format(%d)", color_format);
      return;
    }
  }
/*
...
static XbmcThreads::ConditionVariable  condVar;
...
void CDVDVideoCodecHybris::InitSurfaceTexture(void) {
  ...
  if( g_mainthread == (void*)pthread_self() ) {
    ...
    INFO( "%s::%s::%d - m_textureId = %x", CLASSNAME, __func__, __LINE__,(unsigned int)m_textureId);
    ...
    INFO( "%s::%s::%d - decoding_session = %x", CLASSNAME, __func__, __LINE__,(unsigned int)decoding_session);

    IGBCWrapperHybris igbc_wrapper = 
                              decoding_service_get_igraphicbufferconsumer();
    INFO( "%s::%s::%d - igbc_wrapper = %x", CLASSNAME, __func__, __LINE__,(unsigned int)igbc_wrapper);
    m_surfaceTexture = gl_consumer_create_by_id_with_igbc(m_textureId,
                              igbc_wrapper );
    INFO( "%s::%s::%d - m_surfaceTexture = %x", CLASSNAME, __func__, __LINE__,(unsigned int)m_surfaceTexture);
    ...
    INFO( "%s::%s::%d - m_frameAvailable = %x", CLASSNAME, __func__, __LINE__,(unsigned int)&(*m_frameAvailable));
    IGBPWrapperHybris igbp = decoding_service_get_igraphicbufferproducer();
    INFO( "%s::%s::%d - igbp = %x", CLASSNAME, __func__, __LINE__,(unsigned int)igbp);
    m_surface = surface_texture_client_create_by_igbp(igbp);
    INFO( "%s::%s::%d - m_surface = %x", CLASSNAME, __func__, __LINE__,(unsigned int)m_surface);
    ...
    g_mainHandler->obtainMessage(TMSG_CALLBACK,(int32_t)callbackData,0)->sendToTarget();

    condVar.wait(critSec);
  }
  return;
}
